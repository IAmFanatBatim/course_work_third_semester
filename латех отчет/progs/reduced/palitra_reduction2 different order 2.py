...
for cur_c in range (0, key_colors_amount):
    ...
    while True:
        #print("here")
        cur_sum = pixel_counting[cr-sq_rad_r:cr+sq_rad_r+1, cg-sq_rad_g:cg+sq_rad_g+1, cb-sq_rad_b:cb+sq_rad_b+1].sum()
        if cur_sum > portion:
            sq_rad_r-=1
            sq_rad_g-=1
            sq_rad_b-=1
            ...
            break
        if cr - sq_rad_r > 0 and cr + sq_rad_r < 255:
            sq_rad_r += 1
        if cg - sq_rad_g > 0 and cg + sq_rad_g < 255:
            sq_rad_g += 1
        if cb - sq_rad_b > 0 and cb + sq_rad_b < 255:
            sq_rad_b += 1
    pixel_counting[cr-sq_rad_r:cr+sq_rad_r+1, cg-sq_rad_g:cg+sq_rad_g+1, cb-sq_rad_b:cb+sq_rad_b+1] = 0

...
key_colors_previous_clastersize = np.zeros(key_colors_amount)

previous_changed_points = 0
cur_changed_points = 1
...
while (previous_changed_points != cur_changed_points):
    ...
    #print(key_colors)
    #print("\n")
    cur_changed_points = (np.abs(key_colors_clastersize - key_colors_previous_clastersize)).sum() / 2

    #print(key_colors_clastersize)
    #print(key_colors_previous_clastersize)
    print(cur_changed_points, previous_changed_points)

    if (previous_changed_points != cur_changed_points):
        key_colors = key_colors_centroids[::]
        key_colors_centroids = np.zeros((key_colors_amount, 3))
        key_colors_previous_clastersize = key_colors_clastersize[::]
        previous_changed_points = cur_changed_points
        key_colors_clastersize = np.zeros(key_colors_amount)
        cur_changed_points = 0
...
img2.save("../../images/change_criteria_clastered_reduced_" + str(key_colors_amount) + "_" + filename)

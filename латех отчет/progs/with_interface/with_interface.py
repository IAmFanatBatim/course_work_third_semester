...
def createArrayOfThreads(key_colors):
    with open('../../DMC/array_of_threads 2.json') as json_file:
        ...
        for key_color in key_colors:
            ...
            if best_thread not in newed_array:
                newed_array.append(best_thread)
    return newed_array

def repaintThreadsWithCellStep(arrayImgSource, newed_array, cell_size):
    for row in range(0, arrayImgSource.shape[0], cell_size):
        for col in range(0, arrayImgSource.shape[1], cell_size):
            #Find the closest color from key colors
            best_color_ind = 0
            cur_color_distance = getColorDistance(np.array(newed_array[0]["thread_RGB"]), arrayImgSource[row, col])
            for cur_c in range (1, len(newed_array)):
                #print(getColorDistance(np.array(newed_array[cur_c]["thread_RGB"]), arrayImgSource[row, col]))
                if getColorDistance(np.array(newed_array[cur_c]["thread_RGB"]), arrayImgSource[row, col]) < cur_color_distance:
                    cur_color_distance = getColorDistance(np.array(newed_array[cur_c]["thread_RGB"]), arrayImgSource[row, col])
                    best_color_ind = cur_c
            #Change cur pixel color on closest one
            #print(np.array(newed_array[best_color_ind]["thread_RGB"]), arrayImgSource[row, col])
            arrayImgSource[row:row+cell_size, col:col+cell_size] = np.array(newed_array[best_color_ind]["thread_RGB"])
    return arrayImgSource
    ...
    for row in range(0, arrayImgSource.shape[0], cell_size + grid_size):
        for col in range(0, arrayImgSource.shape[1], cell_size + grid_size):
            ...
            for cur_c in range (0, len(key_colors)):
                if np.array_equal(np.array(newed_array[cur_c]["thread_RGB"]), arrayImgSource[row, col]):
...
def addInfoAboutThreads(img, newed_array, cell_size):
    min_size = 18
    ...
    if (img.width < min_len):
        newImgArr = np.full((oldImgArr.shape[0] + min_size_with_fields*math.ceil(len(newed_array)/2) + tiny_field, 2*(min_len+tiny_field), 3), (225, 225, 225))
        ...
        newImgArr = np.full((oldImgArr.shape[0] + min_size_with_fields*math.ceil(len(newed_array)/2), oldImgArr.shape[1], 3), (225, 225, 225))
    ...
    font1 = ImageFont.truetype("../../CrossStitch3/crossstitch3.ttf", size=cell_size*0.8)
    font2 = ImageFont.truetype("arial.ttf", size=min_size)
    alphabet = "QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm!@#$%^&*()[]{};:',.<>/?1234567890"
    written_threads = 0
    while written_threads < len(newed_array):
        newImgDraw.rectangle((x_pos, y_pos, x_pos+cell_size, y_pos+cell_size), fill=tuple(newed_array[written_threads]["thread_RGB"]))
        newImgDraw.text((x_pos+cell_size*0.1, y_pos+cell_size*0.1), alphabet[written_threads], font=font1, fill=(0, 0, 0))
        newImgDraw.text((x_pos+cell_size+tiny_field, y_pos), newed_array[written_threads]["thread_code"] + newed_array[written_threads]["thread_name"], font=font2, fill=(0, 0, 0))
        x_pos += min_len
...
window = Tk()
path_to_picture = ""
width_reading = Entry(window,width=10)
height_reading = Entry(window,width=10)
colors_amount_reading = Entry(window,width=10)

def findFile():
    filetypes = (("Графический файл PNG", "*.png"),)
    path_to_picture = fd.askopenfilename(title="Открыть файл", filetypes=filetypes)
    
def generate(path_to_picture):
    if (path_to_picture == ""):
        print("Choose file!")
    else:
        print("YEAA!")
        width_in_crosses = width_reading.get()
        if width_in_crosses == "":
            width_in_crosses = 0
        else:
            width_in_crosses = int(width_in_crosses)
        height_in_crosses = height_reading.get()
        ...
        newed_array = createArrayOfThreads(key_colors)

        arrayImg = repaintThreadsWithCellStep(arrayImg, newed_array, cell_size)
        ...
        imgWithPicto = symbolsWithCellAndGridStep(imgWithPicto, arrayImg2, newed_array, cell_size)
        img3 = addInfoAboutThreads(img3, newed_array, cell_size)     
        img3.save("../../images/refactoring1/with_interface.png")


getFile = Button(window, text="Выбрать изображение", command=findFile)
proceed = Button(window, text="Сгенерировать", command=generate(path_to_picture))


getFile.grid(row=0, column=2)
width_reading.grid(row=0, column=0)
height_reading.grid(row=0, column=1)
colors_amount_reading.grid(row=1,column=0)

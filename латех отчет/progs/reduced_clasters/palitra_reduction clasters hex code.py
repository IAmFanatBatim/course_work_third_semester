...
#Return a string with num in hexadecimal number system
def tohex (num):
    hexstr = ""
    add_letters = "ABCDEF"
    while num > 0:
...
#Return a number by string in hexadecimal number system
def fromhex (hexstr):
    num = 0
...
#Return a string with colorcode in hex RRGGBB format
...
#Return an array [R, G, B] by colorcode in hex RRGGBB format
...
#Do until arrays of key colors and centroinds will be equal
...
img2.save("../../images/reduced_clasters/HEXcode_K-means_reduced_" + str(key_colors_amount) + "_" + filename)
